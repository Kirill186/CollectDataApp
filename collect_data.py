...
def generate_ai_project_via_openai(
    ...
    structure = [str(p.relative_to(human_root)).replace("\\", "/") for p in py_files[:150]]
    imports_map = collect_imports(py_files[:150], human_root)

    payload = {
        "model": model,
        "input": [
            {"role": "system", "content": "You are a code generator."},
            {"role": "user", "content": make_prompt(repo_name, structure, imports_map)},
        ],
...
def local_ai_fallback(human_root: Path, ai_sample_dir: Path) -> None:
    ...
    selected = py_files[: min(15, len(py_files))]

    for src in selected:
        rel = src.relative_to(human_root)
        dst = ai_sample_dir / rel
        dst.parent.mkdir(parents=True, exist_ok=True)
        original = src.read_text(encoding="utf-8", errors="ignore")
        transformed = (
            '"""AI-styled synthetic variant.\nGenerated for dataset bootstrap.\n"""\n\n'
            + original
            + "\n\n# synthetic_note = 'non-functional variant'\n"
        )
